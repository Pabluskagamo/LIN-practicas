...
module_param(timer_period, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(timer_period, "Timer expiration time");
...
static int timer_state = 1;
static unsigned long expiration;
...
static irqreturn_t gpio_irq_handler(int irq, void *dev_id)
{
  ...
  last_interrupt = jiffies;
    ...
    if(timer_state == 1){
        ...
        timer_state = 1;
        mod_timer(&my_timer, jiffies + expiration);
    }
...
static void fire_timer(struct timer_list *timer)
{
    ...
    mod_timer(timer, jiffies + expiration);
}
...
/* Create timer */
    ...
    expiration = msecs_to_jiffies(timer_period);

    my_timer.expires = jiffies + expiration; /* Activate it one second from now */
