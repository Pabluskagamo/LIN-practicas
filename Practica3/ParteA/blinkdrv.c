			  ...
			  size_t len, loff_t *off)
{
	...
	char kbuf[MAX_K];
	unsigned int leds[]={0x000000, 0x000000, 0x000000, 0x000000,0x000000,0x000000,0x000000,0x000000};
	char *token;
	char* puntero;
	...
	for(i = 0; i<NR_LEDS;i++){
		...
		message[1]=0x00;
		message[2]=0; 
		message[3]=((leds[i]>>16) & 0xff);
		message[4]=((leds[i]>>8) & 0xff);
		message[5]=(leds[i] & 0xff);
